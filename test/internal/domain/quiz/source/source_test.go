...
func TestNewSource(t *testing.T) {
	...
	t.Run("успешное создание", func(t *testing.T) {
		// Arrange & Act
		s, err := source.NewSource(validBankID, validCriteria)
	...
	t.Run("ошибка при пустом UUID банка (uuid.Nil)", func(t *testing.T) {
		// Arrange & Act
		_, err := source.NewSource(uuid.Nil, validCriteria)

		// Assert
		require.Error(t, err)
		assert.ErrorIs(t, err, source.ErrInvalid)
	...
	t.Run("ошибка при nil criteria", func(t *testing.T) {
		...
		// Act
		_, err := source.NewSource(validBankID, nilCriteria)
