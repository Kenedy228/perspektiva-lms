...
func validateNotEmpty(value string) error {
	if strings.TrimSpace(value) == "" {
		return fmt.Errorf("%w: название элемента не может быть пустым", ErrInvalid)
	}
...
func validateValueCharsLimit(value string) error {
	if utf8.RuneCountInString(value) > valueCharsLimit {
		return fmt.Errorf("%w: название элемента не должно превышать %d символов", ErrInvalid, valueCharsLimit)
	}
