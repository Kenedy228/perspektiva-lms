package answer

import (
	"reflect"
	"testing"

	"gitflic.ru/lms/backend/internal/domain/question"
	"gitflic.ru/lms/backend/internal/domain/question/sequence"
	"github.com/google/uuid"
)

func mustOptionID(t *testing.T, id uuid.UUID) OptionID {
	t.Helper()

	got, err := NewOptionID(id)
	if err != nil {
		t.Fatalf("NewOptionID() error = %v", err)
	}

	return got
}

func makeOptionIDs(t *testing.T, n int) []OptionID {
	t.Helper()

	ids := make([]OptionID, 0, n)
	for i := 0; i < n; i++ {
		ids = append(ids, mustOptionID(t, uuid.New()))
	}

	return ids
}

func TestAnswer_Clone(t *testing.T) {
	type fields struct {
		optionIDs []OptionID
	}
	tests := []struct {
		name   string
		fields fields
		want   question.Answer
	}{
		{
			name: "clones non empty answer",
			fields: fields{
				optionIDs: makeOptionIDs(t, 2),
			},
			want: Answer{
				optionIDs: makeOptionIDs(t, 2),
			},
		},
		{
			name: "clones empty answer",
			fields: fields{
				optionIDs: []OptionID{},
			},
			want: Answer{
				optionIDs: []OptionID{},
			},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			a := Answer{
				optionIDs: tt.fields.optionIDs,
			}
			got := a.Clone()
			if !reflect.DeepEqual(got, Answer{optionIDs: tt.fields.optionIDs}) {
				t.Errorf("Clone() = %v, want %v", got, Answer{optionIDs: tt.fields.optionIDs})
			}
		})
	}
}

func TestAnswer_IsEmpty(t *testing.T) {
	type fields struct {
		optionIDs []OptionID
	}
	tests := []struct {
		name   string
		fields fields
		want   bool
	}{
		{
			name: "empty answer",
			fields: fields{
				optionIDs: []OptionID{},
			},
			want: true,
		},
		{
			name: "non empty answer",
			fields: fields{
				optionIDs: makeOptionIDs(t, 1),
			},
			want: false,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			a := Answer{
				optionIDs: tt.fields.optionIDs,
			}
			if got := a.IsEmpty(); got != tt.want {
				t.Errorf("IsEmpty() = %v, want %v", got, tt.want)
			}
		})
	}
}

func TestAnswer_OptionIDs(t *testing.T) {
	type fields struct {
		optionIDs []OptionID
	}
	tests := []struct {
		name   string
		fields fields
		want   []OptionID
	}{
		{
			name: "returns copy of option ids",
			fields: fields{
				optionIDs: makeOptionIDs(t, 2),
			},
			want: makeOptionIDs(t, 2),
		},
		{
			name: "returns empty slice",
			fields: fields{
				optionIDs: []OptionID{},
			},
			want: []OptionID{},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			a := Answer{
				optionIDs: tt.fields.optionIDs,
			}
			got := a.OptionIDs()
			if !reflect.DeepEqual(got, tt.fields.optionIDs) {
				t.Errorf("OptionIDs() = %v, want %v", got, tt.fields.optionIDs)
			}
			if len(got) > 0 && len(tt.fields.optionIDs) > 0 && &got[0] == &tt.fields.optionIDs[0] {
				t.Errorf("OptionIDs() must return cloned slice")
			}
		})
	}
}

func TestNew(t *testing.T) {
	dup := mustOptionID(t, uuid.New())

	type args struct {
		optionIDs []OptionID
	}
	tests := []struct {
		name    string
		args    args
		want    Answer
		wantErr bool
	}{
		{
			name: "valid ids",
			args: args{
				optionIDs: makeOptionIDs(t, 2),
			},
			wantErr: false,
		},
		{
			name: "empty answer is allowed",
			args: args{
				optionIDs: []OptionID{},
			},
			want: Answer{
				optionIDs: []OptionID{},
			},
			wantErr: false,
		},
		{
			name: "error for empty option id",
			args: args{
				optionIDs: []OptionID{{}},
			},
			wantErr: true,
		},
		{
			name: "error for duplicates",
			args: args{
				optionIDs: []OptionID{dup, dup},
			},
			wantErr: true,
		},
		{
			name: "error for too many ids",
			args: args{
				optionIDs: makeOptionIDs(t, sequence.MaxOptionsCount+1),
			},
			wantErr: true,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := New(tt.args.optionIDs)
			if (err != nil) != tt.wantErr {
				t.Errorf("New() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if tt.wantErr {
				return
			}
			if !reflect.DeepEqual(got, Answer{optionIDs: tt.args.optionIDs}) {
				t.Errorf("New() got = %v, want %v", got, Answer{optionIDs: tt.args.optionIDs})
			}
		})
	}
}

func TestNewOptionID(t *testing.T) {
	id := uuid.New()

	type args struct {
		id uuid.UUID
	}
	tests := []struct {
		name    string
		args    args
		want    OptionID
		wantErr bool
	}{
		{
			name: "creates option id",
			args: args{
				id: id,
			},
			want:    OptionID{id: id},
			wantErr: false,
		},
		{
			name: "error for nil uuid",
			args: args{
				id: uuid.Nil,
			},
			want:    OptionID{},
			wantErr: true,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := NewOptionID(tt.args.id)
			if (err != nil) != tt.wantErr {
				t.Errorf("NewOptionID() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("NewOptionID() got = %v, want %v", got, tt.want)
			}
		})
	}
}

func TestOptionID_ID(t *testing.T) {
	id := uuid.New()

	type fields struct {
		id uuid.UUID
	}
	tests := []struct {
		name   string
		fields fields
		want   uuid.UUID
	}{
		{
			name: "returns id",
			fields: fields{
				id: id,
			},
			want: id,
		},
		{
			name: "returns nil for zero value",
			fields: fields{
				id: uuid.Nil,
			},
			want: uuid.Nil,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			o := OptionID{
				id: tt.fields.id,
			}
			if got := o.ID(); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("ID() = %v, want %v", got, tt.want)
			}
		})
	}
}

func Test_validateOptionIDs(t *testing.T) {
	dup := mustOptionID(t, uuid.New())

	type args struct {
		optionIDs []OptionID
	}
	tests := []struct {
		name    string
		args    args
		wantErr bool
	}{
		{
			name: "valid ids",
			args: args{
				optionIDs: makeOptionIDs(t, 2),
			},
			wantErr: false,
		},
		{
			name: "empty slice valid",
			args: args{
				optionIDs: []OptionID{},
			},
			wantErr: false,
		},
		{
			name: "contains empty id",
			args: args{
				optionIDs: []OptionID{{}},
			},
			wantErr: true,
		},
		{
			name: "contains duplicates",
			args: args{
				optionIDs: []OptionID{dup, dup},
			},
			wantErr: true,
		},
		{
			name: "exceeds max",
			args: args{
				optionIDs: makeOptionIDs(t, sequence.MaxOptionsCount+1),
			},
			wantErr: true,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := validateOptionIDs(tt.args.optionIDs); (err != nil) != tt.wantErr {
				t.Errorf("validateOptionIDs() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}

func Test_validateOptionIDsContainsEmpty(t *testing.T) {
	type args struct {
		optionIDs []OptionID
	}
	tests := []struct {
		name    string
		args    args
		wantErr bool
	}{
		{
			name: "no empty ids",
			args: args{
				optionIDs: makeOptionIDs(t, 2),
			},
			wantErr: false,
		},
		{
			name: "has empty id",
			args: args{
				optionIDs: []OptionID{{}},
			},
			wantErr: true,
		},
		{
			name: "empty slice",
			args: args{
				optionIDs: []OptionID{},
			},
			wantErr: false,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := validateOptionIDsContainsEmpty(tt.args.optionIDs); (err != nil) != tt.wantErr {
				t.Errorf("validateOptionIDsContainsEmpty() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}

func Test_validateOptionIDsDuplicates(t *testing.T) {
	dup := mustOptionID(t, uuid.New())

	type args struct {
		optionIDs []OptionID
	}
	tests := []struct {
		name    string
		args    args
		wantErr bool
	}{
		{
			name: "no duplicates",
			args: args{
				optionIDs: makeOptionIDs(t, 2),
			},
			wantErr: false,
		},
		{
			name: "with duplicates",
			args: args{
				optionIDs: []OptionID{dup, dup},
			},
			wantErr: true,
		},
		{
			name: "empty slice",
			args: args{
				optionIDs: []OptionID{},
			},
			wantErr: false,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := validateOptionIDsDuplicates(tt.args.optionIDs); (err != nil) != tt.wantErr {
				t.Errorf("validateOptionIDsDuplicates() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}

func Test_validateOptionIDsMaxCount(t *testing.T) {
	type args struct {
		optionIDs []OptionID
	}
	tests := []struct {
		name    string
		args    args
		wantErr bool
	}{
		{
			name: "within limit",
			args: args{
				optionIDs: makeOptionIDs(t, sequence.MaxOptionsCount),
			},
			wantErr: false,
		},
		{
			name: "exceeds limit",
			args: args{
				optionIDs: makeOptionIDs(t, sequence.MaxOptionsCount+1),
			},
			wantErr: true,
		},
		{
			name: "empty slice",
			args: args{
				optionIDs: []OptionID{},
			},
			wantErr: false,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := validateOptionIDsMaxCount(tt.args.optionIDs); (err != nil) != tt.wantErr {
				t.Errorf("validateOptionIDsMaxCount() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
